...
### v1.2.2
- **修复**：修复远程电脑进入锁屏/密码界面时，本地客户端卡死无响应的问题。
- **优化**：改进断开连接时的弹窗机制，使用延迟显示避免UI线程阻塞。

### v1.2.1
- **修复**：远程桌面现在可以正确接收 Windows 组合键（如 Win+R、Alt+Tab 等），不再由本地主机响应。
- **优化**：主界面工具栏的“连接”下拉菜单现在支持按文件夹分组显示，与左侧列表保持一致。
- **优化**：现在程序会记住侧边栏（连接列表）的显示/隐藏状态，重启后自动恢复。

### v1.2.0
- **新增**：连接和文件夹的拖拽移动功能。
- **新增**：右键菜单增加“移动到...”选项，可快速整理会话结构。
